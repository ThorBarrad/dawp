            ...
            for (var i = 0; i < that.hotkeys.length; i++) {
                if (that.enable[i]) {
                    ...
                    for (var key in that.hotkeys[i]["keys"]) {
                        if (that.hotkeys[i]["keys"][key] == 17 && e.ctrlKey) { continue; } else if (that.hotkeys[i]["keys"][key] == 18 && e.altKey) { continue; } else if (that.hotkeys[i]["keys"][key] == 16 && e.shiftKey) { continue; } else if (e.keyCode == that.hotkeys[i]["keys"][key]) { continue; } else { flag = false; break; }
                    }
